...
class HolonomicPIDController(Node):
    def __init__(self , bot_id , final_threshold = 10.0 , dock = False , drop = False , assignment = None , drop_angle = np.pi/2):
        ...
        self.timer = self.create_timer(0.04, self.control_cb)  # ~30ms = 33 Hz
    ...
    def pose_cb(self, msg):
        ...
        for pose in msg.poses:
            if pose.id == self.robot_id:
                ...
                yaw_in_degrees = pose.w
                yaw_in_radians = math.radians(yaw_in_degrees)
                self.other_bots[pose.id] = [pose.x , pose.y , yaw_in_radians]
    ...
    def return_time_step(self):
        if(0 <= self.goal_index < len(self.goals)):
            print(f"Bot_id : {self.bot_id} : {self.goals[self.goal_index]}")
            return self.goals[self.goal_index][2]
        else:
            return -1
    ...
    def control_cb(self):
        ...
        if(self.halt is True):
            ...
            other_bot_dists = []
            other_bot_poses = []
            for id,pose in self.other_bots.items():
                d = self.dist(self.other_bots[id][:2] , self.current_pose[:2])
                other_bot_dists.append(d)
                other_bot_poses.append(self.other_bots[id][:2])
            
            target_vec = (other_bot_dists[1] * np.array(other_bot_poses[0]) + other_bot_poses[0] * np.array(other_bot_dists[1]))/(other_bot_dists[0] + other_bot_dists[1])
            ...
            if((self.drop is False) and (self.dock is False)):
                if self.target_flag is False:
                    ...
                    target_theta = self.set_target_angle([x , y] , [target_vec[0],target_vec[1]]) 
...
class fsm(Node):
    def __init__(self , exec):
        ...
        self.timer = self.create_timer(0.01 , self.update_time)
    ...
    def update_time(self):
        if self.timer_start is not None:
            now = self.get_clock().now().nanoseconds/1e9
            if(now - self.timer_start > 0.75 * (self.time_step + 1)):
                self.time_step += 1
        ...
        if(self.state_idx_0 >= len(self.states_order)):
            ...
            if(self.robot_0_planned is False):
                ...
                if self.robot_0_goals is not None:
                    ...
                    reserve_path(self.robot_0_goals , self.reserved_vertices , self.reserved_edges , bot_id , drop = True)
    ...
    def update_2(self , bot_id = 2 , thres = 215):
        ...
        if(self.state_idx_2 >= len(self.states_order)):
            ...
            if(self.robot_2_planned is False):
                ...
                if(self.robot_2_goals is not None):
                    reserve_path(self.robot_2_goals , self.reserved_vertices , self.reserved_edges , bot_id , drop = True)
        ...
        if(self.state_idx_4 >= len(self.states_order)):
            ...
            if(len(self.robots) == self.num_robots and len(self.tasks) == self.num_crates):
                if(self.task_allocated is False):
                    ...
                    if(self.robot_4_goals is not None):
                        ...
                        self.robot_4_goals[-1] = (tx,ty,self.robot_2_goals[-1][2])
            ...
            if(self.robot_4_planned is False):
                ...
                if(self.robot_4_goals is not None):
                    ...
                    reserve_path(self.robot_4_goals , self.reserved_vertices , self.reserved_edges , bot_id , drop = True)
...
def reserve_path(path,reserved_vertices,reserved_edges,robot_id,drop = False):
    def f_surr(x,y,t,last = False):
        ...
        for j in range(61):
            for k in range(61):
                ...
                if distance <= 6:
                    ...
                    if drop is True and last is True:
  
                        for pt in extra_points:
                            reserved_vertices[(int(pt[0]//40) , int(pt[1]//40) , t)] = robot_id
    ...
    D1_3 = np.array([1410.0,1075.0])
    D1_4 = np.array([1020.0,1075.0])
    def give_drop_boundries(c1 = D1_1,c2 = D1_2 ,c3 = D1_3, c4 = D1_4):
            pts = []
    ...
    extra_points = give_drop_boundries()
