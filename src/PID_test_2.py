...
class HolonomicPIDController(Node):
    def __init__(self , bot_id , final_threshold = 10.0 , dock_or_drop = True , assignment = None , drop_angle = np.pi/2):
    ...
    def control_cb(self):
        ...
        if(self.halt is True):
            ...
            #     print(f"Length : {len(self.goals)} ------- Idx : {self.goal_index}")
            ...
            if(dist < self.final_threshold):
                ...
                if(abs(error_theta) < 0.1 and (self.detection or self.drop_or_dock)  is True):
