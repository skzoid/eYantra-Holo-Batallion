...
class PoseDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        #       - Use pixel_to_world() to convert to world coordinates
            ...
            for i in range(len(corners)):
                ...
                w_x,w_y = tvecs[i][0][0],tvecs[i][0][1]
                scale = 1 - 50.0/2438.4
                ...
                elif(ids[i][0] in [12,14,30,13,16]):
    ...
    def publish_crate_poses(self, poses):
        ...
        for id , pose in zip(ids , poses_list):
            ...
            pose_object.x = float(pose[0])
            pose_object.y = float(pose[1])
    ...
    def publish_bot_poses(self, poses):
        ...
        for id , pose in zip(ids , poses_list):
            pose_object = Pose2D()
            pose_object.id = int(id)
            pose_object.x = self.arena_len/2 + float(pose[0])
            pose_object.y = self.arena_len/2 + float(pose[1])
            pose_object.w = float(pose[2]) * 180 / np.pi
            if(pose_object.w < 0):
                pose_object.w += 360
