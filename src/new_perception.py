...
#previous_bot_poses = None
previous_crate_poses = None
...
class PoseDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        """
        global previous_bot_poses
        global previous_crate_poses
        ...
        #       - Use pixel_to_world() to convert to world coordinates
            ...
            rvecs, tvecs, _ = self.custom_estimatePoseSingleMarkers(corners , 45 , self.camera_matrix , self.dist_coeffs)
            crate_poses = {}
            bot_pose = {}  
            ...
            for i in range(len(corners)):
                ...
                scale = 1 - 50.0/2438.4
                bot_scale = 1 - 95/2438.4
                if(ids[i][0] in [0,2,4]):
                    bot_pose[ids[i][0]] = [ w_x * bot_scale  , w_y * bot_scale  , yaw]

                elif(ids[i][0] in [1,3,5,7]):
                    crate_poses[ids[i][0]] = [ w_x  , w_y  , yaw ]
                elif(ids[i][0] in [12,14,30,13,16,21]):
                    crate_poses[ids[i][0]] = [ w_x * scale , w_y * scale  , yaw ]

                
        if previous_crate_poses:
            # for id in bot_pose:
            #     if id in previous_bot_poses:
            #         prev_x, prev_y, prev_w = previous_bot_poses[id]
            #         curr_x, curr_y, curr_w = bot_pose[id]
            #         # if abs(curr_x - prev_x) > 500 or abs(curr_y - prev_y) > 500 or abs(curr_w - prev_w) > 90:
            #         #     bot_pose[id] = previous_bot_poses[id]
            #         if abs(curr_w - prev_w) > 0.1:
            #             bot_pose[id] = previous_bot_poses[id]

            for id in crate_poses:
                if id in previous_crate_poses:
                    prev_x, prev_y, prev_w = previous_crate_poses[id]
                    curr_x, curr_y, curr_w = crate_poses[id]
                    # if abs(curr_x - prev_x) > 500 or abs(curr_y - prev_y) > 500 or abs(curr_w - prev_w) > 90:
                    #     crate_poses[id] = previous_crate_poses[id]  
                    if abs(curr_w - prev_w) > (np.pi/5):
                        crate_poses[id] = previous_crate_poses[id]    
        
        previous_crate_poses = crate_poses
        ...
        undistorted_image = cv2.resize(undistorted_image , (600,600))
    ...
    def custom_estimatePoseSingleMarkers(self , corners, markerLength, cameraMatrix, distCoeffs):
        """
        ...
        obj_points = np.array([
            [-half_len,  half_len, 0.0],  # top-left
            [ half_len,  half_len, 0.0],  # top-right
            [ half_len, -half_len, 0.0],  # bottom-right
            [-half_len, -half_len, 0.0],  # bottom-left
        ], dtype=np.float32)
