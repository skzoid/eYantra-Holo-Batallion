...
class HolonomicPIDController(Node):
    def __init__(self , bot_id , final_threshold = 10.0 , dock_or_drop = True , assignment = None):
        ...
        # ---------------- PID Parameters ----------------
        self.pid_params = {
            'x': {'kp': 15.0, 'ki': 0.00, 'kd': 0.0, 'max_out': self.max_vel},
            'y': {'kp': 15.0, 'ki': 0.00, 'kd': 0.0, 'max_out': self.max_vel},
        ...
        if(self.halt is True):
            ...
            if(self.target_flag is False):
                ...
                if(self.goal_index < len(self.goals) - 1):
                    ...
                    target_x = self.goals[len(self.goals)-1][0]
                    target_y = self.goals[len(self.goals)-1][1]
            else:
